...
import {deletedBlockedCountries} from '../shared/jsons/deletedCountriesBlocked'
...
const blockedCountries = ref<Country[]>([])
...
export const useWorld = () => {
    ...
    const fetchCountries = async (shouldDeleteBannedCountries: boolean = false) => {
        ...
        new WorldService().getCountries().then((resp: Country[]) => {
            ...
            blockedCountries.value = resp;

            if (shouldDeleteBannedCountries) {
                countries.value = deleteUnavailableCountries(resp)
            }
            blockedCountries.value = deleteBlockedCountries(resp)
    ...
    const deleteBlockedCountries = (blockedCountries: Country[]): Country[] => {
        return blockedCountries.filter(country => !deletedBlockedCountries().countries_blocked.includes(country.name.toUpperCase().trim())).sort((a, b) => a.name.localeCompare(b.name))
    }
